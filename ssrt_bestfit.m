function [overallMeanSSRT, meanSSRT, meanIntSSRT, meanIntSSRT_bf, HanesSSRT, meanInhibFunc, Time, BestWFit, FitValues] = ...
    ssrt_bestfit(NSSDist, inhib_func, SSDall, tachomc_pw)

% written by L. Boucher March 2003 as SSRT_LB_bestfit.m
% slight changes by VP 12/2012-09/2014
%======================================================================
% INPUTS:
...
rankedNSS_RT = rankedNSS_RT(~isnan(rankedNSS_RT));
...
if meanSSRT<tachomc_pw %try sigmoid fit
    [fitresult, ~] = sigmoidfit(SSDall, inhib_func);
    BestSFit = fitresult(Time);
    FitValues=BestSFit(SSDall-(min(SSDall))+1);
    for x = 2:1:(length(Time)-1)
        val(x-1) = (BestSFit(x) - BestSFit(x-1)) * Time(x);
    end
    meanInhibFunc = sum(val)/(max(FitValues) - min(FitValues));
    meanSSRT = max(meanSSRT, meanNSS - meanInhibFunc);
end
...
% Overall Stats
%======================================================================
overallMeanSSRT = [meanSSRT meanIntSSRT meanIntSSRT_bf];
overallMeanSSRT=overallMeanSSRT(overallMeanSSRT>tachomc_pw);
overallMeanSSRT=nanmean(overallMeanSSRT);
% round to 2 decimal places
meanIntSSRT = (ceil(meanIntSSRT*1000))/1000;
meanSSRT = (ceil(meanSSRT*1000))/1000;
meanIntSSRT_bf = (ceil(meanIntSSRT_bf*1000))/1000;
