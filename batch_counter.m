...
% end
    ...
    % read A (File Name) 
    [~,filestoload] = xlsread([directory 'cmd.xlsx'],1,['A2:A' num2str(numrows)]);

    %% pre-alloc
alldata=struct('meanIntSSRT',[],'meanSSRT',[],'overallMeanSSRT',[],...
    'inhibfun',[],'SSDs',[]);
% 'dir',[],'NSSsacdelay',[],'NSSsuccessrate',[],'SRsacdelay',[],,'meanRT',[],...
%     't_inhibfun',[],'NRMSE',[],'monotonic',[],,...
%     'simpleSSRT',[],'substrSSRT',[]

%% run analysis - first pass
for filenum=1:length(filestoload)
      
if strcmp('R',filestoload{filenum}(1))
   subject='Rigel';
   procdir = [directory,'processed',slash,'Rigel',slash];
elseif strcmp('S',filestoload{filenum}(1))
   subject='Sixx';
   procdir = [directory,'processed',slash,'Sixx',slash];
elseif strcmp('H',filestoload{filenum}(1))
   subject='Hilda';
...
loadfile=procdirfileNames(~cellfun('isempty',regexpi(procdirfileNames,filestoload{filenum},'match')));
...
[overallMeanSSRT,meanIntSSRT,meanSSRT,inhibfun,ssds]=findssrt(loadfile{:});
alldata(filenum).overallMeanSSRT=round(overallMeanSSRT);
alldata(filenum).meanIntSSRT=round(meanIntSSRT);
alldata(filenum).meanSSRT=round(meanSSRT);
alldata(filenum).inhibfun=inhibfun;
alldata(filenum).SSDs=ssds;

if ~isnan(overallMeanSSRT)
%     plot(ssds,inhibfun,'Color',[0.25 0.25 0.25],'LineWidth',1.8);
%     % hold on
...
%     set(gca,'Xlim',[50 300],'XTick',100:50:250,'TickDir','out','box','off'); %'XTickLabel',[50:50:400]
...
% [alldata(~arrayfun(@(x) isnan(x.overallMeanSSRT),alldata)).overallMeanSSRT]
% [alldata(~arrayfun(@(x) isnan(x.meanIntSSRT),alldata)).meanIntSSRT]
% [alldata(~arrayfun(@(x) isnan(x.meanSSRT),alldata)).meanSSRT]

%% run analysis: second pass - align data
% keep file with appropriate SSRT
goodsess=filestoload([alldata.meanSSRT]>60 & [alldata.meanSSRT]<140);
...
% process file, aligning to sac, directions collapsed
%alignbh=get(findobj('Tag','aligntimepanel'),'children');
%set(findobj('Tag','aligntimepanel'),'SelectedObject',alignbh(strcmp(get(alignbh,'tag'),'mainsacalign')))
...
% 
% % final, overall value
% foSSRT=mean([nanmean(oSSRTs(oSSRTs>50 & oSSRTs<150))...
