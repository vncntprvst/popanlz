...
[~,resort]=sort(recordingList);[~,resort]=sort(resort);gsdata.fileNames=fileNames(resort);

% check behavioral data: keep only ones with correct behavior data
badapl=isnan(clusterIdx) | cellfun('isempty',gsdata.allsacdelay);

fn = fieldnames(gsdata);
for lp=1:length(fn)
    gsdata.(fn{lp})=gsdata.(fn{lp})(~badapl,:);
end
...
%         [compgssdf,clusgsndata,startstop]=Figure4_PopSDFs(gsdata,proc_option,CCNdb);
%         
...
%         Figure4_PopSDFs_SaccadeAlignment(gsdata,proc_option,CCNdb)
%         Figure4_PopSDFs_StopSignalAlignment(gsdata,proc_option,CCNdb)
        pop_a_countermanding(gsdata,proc_option,CCNdb);
