...
%                   PopClustering_GUI('dataset','cDn_gsdata') %loads cDn_gsdata
%
% See also: GUIDE, GUIDATA, GUIHANDLES

% Edit the above text to modify the response to help PopClustering_GUI

% Last Modified by GUIDE v2.5 29-Dec-2015 19:49:34
...
clusterProfiles=cell(length(unique(data.clusteridx)),1);
for mclussp=1:length(unique(data.clusteridx))
%      %findobj('Tag',['cluster' num2str(mclussp)])
    clusterTag=['cluster' num2str(mclussp)];
%     % add waveform to handle structure
    handles.([clusterTag '_wf'])= nanmean(handles.unitsProfile(handles.clusterIdx==clusid(mclussp),:));
...
%      text(100,mean(get(gca,'ylim')),['n=' num2str(sum(data.clusteridx==clusid(mclussp)))]);
%     xlabel('Time')
...
plot(handles.mainplot,handles.unitsProfile(handles.currentUnitDisplay,:),'Color',...
...
handles.([clusterTag '_wf'])= nanmean(handles.unitsProfile(handles.clusterIdx==clusid(clusNum),:));
% update cluster plot
axes(handles.(clusterTag));
