function sing_sess_countermanding(fileName)
...
%     end
    ...
    cmdData.file=fileName;
end

if strcmp('R',cmdData.file(1))
    subject='Rigel';
    procdir = [directory,'processed',slash,'Rigel',slash];
elseif strcmp('S',cmdData.file(1))
    subject='Sixx';
    procdir = [directory,'processed',slash,'Sixx',slash];
elseif strcmp('H',cmdData.file(1))
    subject='Hilda';
...
loadfile=procdirfileNames(~cellfun('isempty',regexpi(procdirfileNames,cmdData.file,'match')));
...
mainfig=figure; cmap=colormap('lines');
