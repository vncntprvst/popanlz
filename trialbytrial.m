...
behav(:,5)=cellfun(@(x) x(1).eyevel, gsdata.allndata(~cellfun('isempty',gsdata.allsacdelay),1),'UniformOutput',false);%eye velocity
...
for rec=1:size(neur,1)
    ...
    for trial=1:size(neur{rec,2},1)
        ...
        tr_wd_conv = gauss_filtconv(neur{rec,2}(trial,neur{rec,3}-500:neur{rec,3}-1),50);
        %         plot(conv);
        wd_shift=500-find(tr_wd_conv==max(tr_wd_conv));% plot(tr_wd_conv(1:end-wd_shift));
        neur{rec,5}(trial)={glmfit(1:500-wd_shift,tr_wd_conv(1:end-wd_shift))};
...
[ccoefs,sigcc]=cellfun(@(x,y) corrcoef(x(2,:),y),cellfun(@(x) [x{:}], neur(:,5),'UniformOutput',false),behav(:,3),'UniformOutput',false);
...
for rec=1:size(neur,1)
    ...
    [neur{rec,6},~,neur{rec,7}]=conv_raster(neur{rec,2}(:,neur{rec,3}-1900:neur{rec,3}+3*sigma),sigma);
