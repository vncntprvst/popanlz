...
% get cluster indices
[sorted_unit_ids,sunitid_idx]=sort(cellfun(@(x) x.unit_id, cmdata.alldb));
...
sunitid_revidx(sunitid_idx)=1:length(cellfun(@(x) x.unit_id, cmdata.alldb));%reverse index
...
neur(:,2,1)=cellfun(@(x) x(1).rast, cmdata.allndata(~cellfun('isempty',cmdata.allsacdelay), 1),...
    'UniformOutput',false); %NSS trials rast, aligned to sac
neur(:,2,2)=cellfun(@(x) x(1).rast, cmdata.allndata(~cellfun('isempty',cmdata.allsacdelay), 2),...
    'UniformOutput',false); %NSS trials rast, aligned to tgt
neur(:,3,1)=cellfun(@(x) x(1).alignt, cmdata.allndata(~cellfun('isempty',cmdata.allsacdelay), 1),...
    'UniformOutput',false); %NSS trials alignt
neur(:,4,1)=cellfun(@(x) x(1).evttime, cmdata.allndata(~cellfun('isempty',cmdata.allsacdelay), 1),...
    'UniformOutput',false); %NSS trials event time. Reminder of events: 'cue' 'eyemvt' 'fix' 'rew' 'fail'
neur(:,4,2)=cellfun(@(x) x(1).evttime, cmdata.allndata(~cellfun('isempty',cmdata.allsacdelay), 2),...
    'UniformOutput',false); %NSS trials event time. Reminder of events: 'cue' 'eyemvt' 'fix' 'rew' 'fail'
behav=cellfun(@(x) x(1).trialnb, cmdata.allndata(~cellfun('isempty',cmdata.allsacdelay), 1),...
...
behav(:,5)=cellfun(@(x) x(1).eyevel, cmdata.allndata(~cellfun('isempty',cmdata.allsacdelay),2),...
